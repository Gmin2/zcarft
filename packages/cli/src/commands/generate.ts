...
export default class Generate extends Command {
  ...
  static override examples = [
    ...
    '<%= config.bin %> <%= command.id %> ./FHECounter.abi.json --name FHECounter',
    '<%= config.bin %> <%= command.id %> ./FHECounter.abi.json --output ./custom-tasks',
  ];

  static override args = {
    abiPath: Args.string({
      description: 'Path to contract ABI file (JSON)',
      required: true,
    }),
  };
